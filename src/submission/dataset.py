...
class NameDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = inp + self.MASK_CHAR + oup + self.MASK_CHAR       # 先将答案打上mask
        x = x + self.PAD_CHAR*(self.block_size - len(x))      # 然后将答案后面余下的打上pad
        y = self.PAD_CHAR*(len(inp)-1) + x[len(inp):]         # 然后对y前面打上pad，后面答案部分与改造后的x一致
        
        x = x[:-1]                                            # 同理，最后一个字符去掉（因为是要做预测），y不需要去头因为头部已经是pad了
